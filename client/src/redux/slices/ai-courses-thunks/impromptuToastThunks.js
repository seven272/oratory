...
export const fetchStartImpromptuToast = createAsyncThunk(
  ...
  async ({ courseCode, exerciseData }, { rejectWithValue }) => {
    try {
      const res = await axiosInstance.post('/courses/simulate/hr-storm/impromptu-toast/start', { courseCode, exerciseData })
      return res.data
    } catch (error) {
      return rejectWithValue(error.response?.data || { message: 'Ошибка старта тренажера экспромта' })
    }
  }
)
...
export const fetchSendImpromptuResponse = createAsyncThunk(
  ...
  async ({ courseCode, audioBlob }, { rejectWithValue }) => {
    try {
      ...
      const res = await axiosInstance.post('/courses/simulate/hr-storm/impromptu-toast/respond', formData, {
...
export const fetchFinishImpromptuToast = createAsyncThunk(
  ...
  async ({ courseCode }, { rejectWithValue }) => {
    try {
      const res = await axiosInstance.post('/courses/simulate/hr-storm/impromptu-toast/finish', { courseCode })
      return res.data
    } catch (error) {
