// ai-courses-simulators/party-charisma/bar-small-talk/barSmallTalkController.js
...
const startBarSmallTalkTrainer = async (req, res) => {
  try {
    ...
    const preview = `Вы подходите к бару. Собеседник: ${exerciseData.role}. Контекст: ${exerciseData.context}`
    const question = `${exerciseData.firstQuestion}`

    // Записываем структуру сессии
    progress.blocksProgress.aiWorkout.currentSession = {
      status: 'active',
      workoutConfigId: 'bar_small_talk',
      exerciseData,
      messages: [],
      createdAt: new Date(),
    }
    ...
    console.error('Error in startBarSmallTalkTrainer:', error)
    res.status(500).json({
      message: 'Ошибка сервера при старте разговора у бара',
      error: error.message,
    })
  }
}
...
const generateBarSmallTalkResponse = async (req, res) => {
  try {
    ...
    if (session.status !== 'active' || session.workoutConfigId !== 'bar_small_talk') {
    ...
    if (req.file) {
      try {
        ...
        console.error('Ошибка Yandex SpeechKit в bar_small_talk:', speechError)
        return res.status(500).json({
          message: 'Не удалось распознать речь. Пожалуйста, повторите запись.',
    ...
    if (!userMessage || !userMessage.trim()) {
      ...
      return res.status(200).json({
        answer: 'Вы промолчали. Собеседник у бара сделал глоток и вопросительно посмотрел на вас.',
        isSessionFinished,
        isError: true,
    ...
    const PROMPT = getBarSmallTalkDialogPrompt(role, topic, context, cleanUserMessage)
    ...
    console.error('Ошибка в generateBarSmallTalkResponse:', error)
    res.status(503).json({ answer: 'В баре стало слишком шумно. Собеседник переспросил, что вы сказали.' })
  }
}
...
const finishBarSmallTalkTrainer = async (req, res) => {
  try {
    ...
    const session = progress.blocksProgress?.aiWorkout?.currentSession
    if (!session || session.workoutConfigId !== 'bar_small_talk') {
    ...
    if (meaningfulMessages.length < 2) {
      ...
      return res.status(200).json({
        message: 'Диалог завершен без оценки.',
        totalScore: 0,
        feedback: 'Слишком много пауз и пропущенных реплик. Собеседник отвернулся к бару.',
        progressData: progress,
      })
    }

    const chatHistory = session.messages
      .map((m) => `${m.role === 'user' ? 'Пользователь' : 'Собеседник'}: ${m.text}`)
      .join('\n')
    ...
    try {
      const response = await gigachatAxiosClient.post('/chat/completions', {
        model: 'GigaChat-2',
        messages: [
          { role: 'system', content: BAR_SMALL_TALK_EVALUATION_PROMPT },
          { role: 'user', content: `Проанализируй этот неформальный Small Talk:\n${chatHistory}` },
        ],
        max_tokens: 700,
      })
      ...
      evaluation = parseAiResponse(aiJsonResult, {
        iceBreaking: 40,
        conversationalFlow: 40,
      })
    } catch (apiError) {
      console.error('Сбой GigaChat при оценке bar_small_talk:', apiError.message)
    }

    if (!evaluation) {
      evaluation = {
        totalScore: 50,
        feedback: 'Ваш разговор принят. Детальный разбор недоступен из-за технических накладок.',
        criteria: { iceBreaking: 50, conversationalFlow: 50 },
      }
    }
    ...
    if (progress.blocksProgress.aiWorkout.accumulatedScore >= globalRequiredScore) {
      ...
      progress.currentBlockIndex = 2 // Перевод на Блок 3: IRL-челлендж
    } else {
    ...
    console.error('Ошибка в finishBarSmallTalkTrainer:', error)
