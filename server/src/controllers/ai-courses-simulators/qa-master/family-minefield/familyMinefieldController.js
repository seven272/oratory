...
} from './familyMinefieldPrompt.js'

const startFamilyMinefieldTrainer = async (req, res) => {
  try {
    ...
    const preview = `Вы сидите за праздничным семейным столом. Роль вашего оппонента: "${exerciseData.role}". ${exerciseData.context}. \nВнезапно повисает пауза, и родственник обращается к вам с неудобным вопросом...`
    const question = `${exerciseData.firstQuestion}`

    progress.blocksProgress.aiWorkout.currentSession = {
      status: 'active',
      workoutConfigId: 'family_minefield',
      exerciseData,
      messages: [],
      createdAt: new Date(),
    }
    ...
    console.error('Error in startFamilyMinefieldTrainer:', error)
    res
      .status(500)
      .json({
        message:
          'Ошибка сервера при старте тренажера Семейное минное поле',
        error: error.message,
      })
  }
}

const generateFamilyMinefieldResponse = async (req, res) => {
  try {
    const userId = req.userId
    ...
    if (req.file) {
      try {
        ...
        console.error(
          'Ошибка STT в тренажере Семейное минное поле:',
          speechError,
        )
        return res
          .status(500)
    ...
    if (!userMessage || !userMessage.trim()) {
      ...
      return res.status(200).json({
        answer:
          'Вы промолчали. Родственник хмыкнул и за столом повисла неловкая тишина. Все ждут вашей реакции.',
    ...
    const PROMPT = getFamilyMinefieldDialogPrompt(
      role,
      topic,
      context,
      cleanUserMessage,
    ...
    if (!aiAnswer) {
      throw new Error(
        'Пустой ответ от GigaChat в тренажере Семейное минное поле',
      )
    }
    ...
    console.error(
      'Ошибка в generateFamilyMinefieldResponse:',
      error.message,
    )
    res
      .status(503)
      .json({
        answer:
          'Кто-то из гостей громко звякнул тарелкой. Пожалуйста, повторите вашу фразу.',
      })
  }
}

const finishFamilyMinefieldTrainer = async (req, res) => {
  try {
    const userId = req.userId
    const { courseCode } = req.body
    ...
    if (meaningfulMessages.length < 2) {
      ...
      return res.status(200).json({
        ...
        totalScore: 0,
        feedback:
          'Вы уклонились от ответов. Границы отстоять не удалось.',
    ...
    const chatHistory = session.messages
      .map(
        (m) =>
          `${m.role === 'user' ? 'Оратор' : 'Родственник'} : ${m.text}`,
      )
      .join('\n')
    ...
    try {
      const response = await gigachatAxiosClient.post(
        '/chat/completions',
        {
          ...
          messages: [
            {
              role: 'system',
              content: FAMILY_MINEFIELD_EVALUATION_PROMPT,
            },
            {
              role: 'user',
              content: `Проанализируй этот стресс-диалог защиты границ за семейным столом:\n${chatHistory}`,
            },
          ],
          max_tokens: 700,
        },
      ...
      console.error(
        'Сбой сети GigaChat при оценке тренажера Семейное минное поле:',
        apiError.message,
      )
    }
    ...
    if (!evaluation) {
      evaluation = {
        totalScore: 50,
        feedback:
          'Ужин окончен. Из-за технических неполадок подробная аналитика временно недоступна.',
        criteria: { emotionalControl: 50, topicShift: 50 },
      }
    }
    ...
    return res.status(200).json({
      ...
      evaluation: {
        ...
        criteria: evaluation.criteria, // { emotionalControl, topicShift }
        isScoreCounted,
      },
    })
  } catch (error) {
    console.error(
      'Ошибка финализации тренажера Семейное минное поле:',
      error,
    )
    return res
      .status(500)
